...
- **Weight** (`src/app/weight/`): daily weigh-ins, one `WeightEntry` per day. `WeeklyAverageRow.tsx` shows each week's average with a trend delta vs. the prior week; incomplete weeks (start of tracking, or missed days) get a day-count badge and expand to reveal `MissingDayRow.tsx` ghost rows for the missing days, loggable inline and prefilled from the most recent entry. `WeightEntryRow.tsx` supports inline editing with quick-adjust steppers, shared between the History list and expanded weekly rows.
...
- **Stats**: weekly calorie/protein averages vs. goals, with custom date-range filtering and quick presets (last month/3 months/year); days with zero calories or protein logged are excluded from an average rather than dragging it down. The selected range persists to `?from=&to=` in the URL (`StatsCharts.tsx`, updated via the History API directly rather than `next/navigation`'s router, since filtering is client-side from already-loaded data and routing would re-trigger the page's `force-dynamic` Prisma query).

Cross-cutting conventions worth knowing: Cancel-before-Save button order in edit-mode forms; dismissible inline error messages (× button) rather than errors that stick around; deleting a `Food`/`Recipe` still referenced elsewhere is blocked with an error naming what references it. "Add new" forms (Add Food, Add Recipe) disable their submit button until required fields are filled, and show a red Cancel button (matching Delete's color) once any field has a value, clearing the whole form in one tap.
...
**Signature element**: the dual progress ring on Today (`src/components/ProgressRing.tsx`) — concentric sage (calories) + terracotta (protein) rings against daily goals, with a calories/protein "remaining" (or "over", flagged in red) readout beneath each ring's totals. This is the one deliberately distinctive visual moment; keep the rest of the UI disciplined/quiet around it rather than adding more decorative flourishes elsewhere.
...
**Responsive pattern for dense grids** (Foods list/rows, Add Food form): rather than duplicating markup per breakpoint, keep one grid and give elements explicit mobile-only placement (`col-start-*`/`row-start-*`/`col-span-*`), reset back to natural auto-flow at `sm:` (`sm:col-auto sm:row-auto`) so the original desktop layout is untouched. Below `sm`, dense tables (`FoodsList.tsx`/`FoodRow.tsx`, `ArchivedFoodsList.tsx`) instead switch to fixed narrow columns inside a horizontally-scrolling card (`overflow-x-auto` + a `min-w-[Nrem]` inner wrapper) rather than trying to squeeze a desktop-width table into phone width.

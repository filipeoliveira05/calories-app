...
**Status**: as of 2026-07-25, all core pages (Foods, Today, Weight, Stats, Settings) are built and deployed to Vercel, with the warm dark-first redesign applied and several rounds of UX polish since (meal-type grouping on Today, weekly weigh-in breakdowns and custom date-range filtering on Stats, alignment/cursor/theming fixes). Prior session added: inline gram editing on Today's logged entries, food categories (`FoodCategory` enum, filterable on the Foods page, grouped in Today's food picker), opt-in unit-based logging for foods that are counted rather than weighed (e.g. "2 yogurts" instead of grams), and quick range presets (last month/3 months/year) on Stats. A later session expanded Settings beyond just calorie/protein numbers: a Personal Info profile (sex, birth date, height, activity level, weight goal type/rate, protein target) that derives suggested calorie and protein goals via Mifflin-St Jeor, using the weekly-average weight already tracked on the Weight page (`getLatestWeeklyAverageWeight` in `src/lib/weeks.ts`, calculation in `src/lib/nutritionGoals.ts`). One "Save & calculate goals" action saves the profile and applies the computed goals in one step, previewed as maintenance/calorie-goal/protein-target lines; manually setting goals directly is still available as a separate, clearly-labeled card below. The following session added recipes: a Recipes tab on the Foods page (`src/app/foods/RecipesList.tsx`, `AddRecipeForm.tsx`, `RecipeCard.tsx`, sharing the ingredient-row UI in `IngredientRows.tsx`) for creating/editing named groups of foods with fixed grams/unit amounts per ingredient, plus a Food/Recipe toggle on Today's `LogMealForm` that previews the ingredient list and total kcal/protein, then logs one `MealEntry` per ingredient via the `logRecipe` server action — each entry is independently editable/removable afterward, same as a manually logged food. Deleting a `Food` still referenced by a recipe is blocked (`onDelete: Restrict`), with the error naming which recipe(s) to edit first. Also fixed that session: the meal-type default was silently reverting to Breakfast after logging, caused by React's `<form action={fn}>` auto-resetting the DOM form on success — `LogMealForm` now submits via a plain `onSubmit` handler instead.

Since then: Today's `DateNav` gained a "Today" jump button, shown only when viewing a past/future day (`src/app/DateNav.tsx`). Recipes can now be tagged with zero or more `MealType`s purely for organization — a filter-pill row and enum-order sorting on the Recipes tab (`src/app/foods/RecipesList.tsx`), edited via `MealTypePicker.tsx`; tagging never restricts which meal type a recipe can actually be logged under. The Food delete-blocked error (`FoodRow.tsx`) is now dismissible instead of sticking on screen forever, and lists both recipes and templates it's used in.

Most recent session added Day Templates, the tier above Recipes: `DayTemplate`/`DayTemplateEntry` (flat food+mealType+amount rows, same shape as `MealEntry`, no live link back to `Recipe`) let a full day's eating pattern (e.g. "Standard Monday") be saved and applied in one click. Foods page gained a Templates tab (`DayTemplatesList.tsx`, `AddDayTemplateForm.tsx`, `DayTemplateCard.tsx`, shared builder in `DayTemplateEntryEditor.tsx`) for building templates from Foods and/or Recipes assigned per meal type, with per-meal-type and whole-day kcal/protein previews while building. Today page gained `DayTools.tsx`: apply a saved template on top of the day's existing entries (warns first if the day isn't empty — applying never replaces or deletes), save the day's current entries as a new template, and clear all of a day's entries. Treat this as a working, deployed app, not a WIP — check git history before assuming something is unbuilt, since each commit corresponds to one feature area with a detailed message.
